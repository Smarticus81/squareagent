...
const queryClient = new QueryClient({
  defaultOptions: {
    queries: { retry: false, refetchOnWindowFocus: false, staleTime: 30_000, gcTime: 5 * 60_000 },
  },
});
    ...
    <Layout>
      <Suspense fallback={<RouteFallback />}>
        <Switch>
          ...
          <Route path="/dashboard"><NavigateReplace to="/assistants" /></Route>
          <Route path="/account"><NavigateReplace to="/settings" /></Route>
          <Route path="/plans"><NavigateReplace to="/pricing" /></Route>
          <Route path="/command"><NavigateReplace to="/assistants" /></Route>
          <Route path="/console"><NavigateReplace to="/assistants" /></Route>
          <Route path="/mission-control"><NavigateReplace to="/autonomy" preserveLocationExtras={false} /></Route>
          ...
          {/* Legacy booking links now remove sales-call friction and preserve attribution. */}
          <Route path="/book-demo"><NavigateReplace to="/signup" /></Route>
    ...
    return (
      <div className="vl-page-shell flex-1 px-4 pb-24 pt-24 sm:px-6 lg:px-10">
        <div className="vl-panel mx-auto max-w-2xl p-8">
          ...
          <h1 className="vl-display mt-3 text-[36px]" style={{ color: "var(--color-vl-ink)" }}>Clerk billing is not configured.</h1>
          <p className="mt-3 text-[14px]" style={{ color: "var(--color-vl-ink-muted)" }}>Add VITE_CLERK_PUBLISHABLE_KEY to enable Clerk subscription management.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="vl-page-shell flex-1 px-4 pb-24 pt-24 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-5xl">
        <SignedIn>
          <div className="mb-6 flex items-center gap-4"><OrganizationSwitcher hidePersonal /></div>
          <OrganizationProfile routing="path" path="/billing" />
        </SignedIn>
        <SignedOut><BillingSignedOut /></SignedOut>
      </div>
    </div>
  );
}
...
function BillingSignedOut() {
  ...
  if (hasToken) {
    return <div className="vl-panel flex items-center gap-3 p-8"><span className="h-4 w-4 animate-spin rounded-lg border-2 border-black/15 border-t-black/60" /><p className="text-[14px]" style={{ color: "var(--color-vl-ink-muted)" }}>Preparing your organization's billing…</p></div>;
  }
  return (
    <div className="vl-panel p-8">
      <p className="vl-eyebrow">Billing</p>
      <h1 className="vl-display mt-3 text-[36px]" style={{ color: "var(--color-vl-ink)" }}>Log in to manage billing.</h1>
      <p className="mt-2 text-[14px]" style={{ color: "var(--color-vl-ink-muted)" }}>Billing lives at the organization level. Log in to your VoyceLab account to view and manage your subscription.</p>
...
function App() {
  ...
  return <ClerkProvider publishableKey={clerkPublishableKey}><AppContent /></ClerkProvider>;
}
