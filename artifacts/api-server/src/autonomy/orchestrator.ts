...
function materiallyDegraded(s: Awaited<ReturnType<typeof collectBusinessSnapshot>>): boolean {
  return (s.product.toolCalls >= 20 && s.product.toolFailureRate >= 0.12) ||
    (s.product.voiceSessions >= 10 && s.product.noSuccessfulToolRate >= 0.2);
}
...
export async function runAutonomyCycle(trigger = "scheduler"): Promise<AutonomyCycleResult> {
  ...
  try {
    ...
    try {
      deliverability = await runDeliveryFailureInbox(runId, 25);
    } catch (error) {
      deliverabilitySafe = false;
      deliverability = { status: "reconciliation_failed", error: error instanceof Error ? error.message : String(error) };
    ...
    if (productHealthy && financeHealthy && outboundEnabled() && deliverabilitySafe) {
      growth.campaign = await ensureOutboundCampaign(before, runId);
      growth.outbound = await runOutboundBatch(runId);
    } else if (!deliverabilitySafe) {
      growth.outbound = { paused: "deliverability_reconciliation_failed" };
...
export async function runAutonomyCycleLocked(trigger = "scheduler"): Promise<AutonomyCycleResult | { enabled: true; skipped: "already_running" }> {
  ...
  try {
    ...
    try { await client.query(`SELECT pg_advisory_unlock(82651231)`); } catch {}
    client.release();
  }
}
