  ...
  return [
    {
      ...
      expectedImpact: ["more verified venue-owner prospects", "more opportunities for paid conversion"],
      executionSteps: ["find real wedding venues, event venues and private-event spaces", "verify the exact public business email on the official site", "prefer owner, GM or events-lead contacts", "rank by likelihood to buy now"],
    },
    {
      agent: "growth",
      ...
      executionSteps: ["address the venue owner or GM", "state that VoyceLab is a Voice AI agent that helps run the venue", "use concrete owner tasks instead of feature jargon", "use Start Free as the only CTA", "measure paid conversion, not replies"],
    },
    {
      agent: "activation",
...
export async function createAutonomyPlan(snapshot: BusinessSnapshot, revenue?: RevenuePressureSnapshot): Promise<AutonomyPlan> {
  ...
  const instructions = [
    ...
    "VoyceLab's primary buyer is the owner or general manager of a wedding venue, event venue, private-event space, banquet property or venue group. Keep growth focused on people responsible for the whole venue. Bars and breweries are secondary only when events are a meaningful part of the business.",
    ...
    "Use blunt owner language: VoyceLab is a Voice AI agent that helps run the venue. Use concrete examples such as inventory counts, sales, open orders, booking/tour inquiries from business email, event details and approved Square actions. Do not claim direct calendar scheduling until that capability exists. Avoid voice layer, orchestration, operational intelligence, workflow transformation, connected systems, AI-powered operations, streamline, unlock, leverage, optimize, ecosystem, and platform transformation.",
