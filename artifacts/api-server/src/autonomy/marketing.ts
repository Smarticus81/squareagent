...
export async function ensureOutboundCampaign(snapshot: BusinessSnapshot, runId?: string): Promise<string | null> {
  ...
  const proposed = await structuredModel<{ campaignThesis: string; variants: CampaignVariantProposal[] }>(
    [
      ...
      "Design exactly three materially different direct-response email angles for OWNERS and GENERAL MANAGERS of wedding venues, event venues and private-event spaces.",
      "VoyceLab is a Voice AI agent that helps run a venue. Owners can speak to it for inventory counts, sales, open orders, approved Square actions, booking/tour inquiries from business email, event details from connected knowledge, and other day-to-day operational answers.",
      "Do not center bartenders, 'less tapping', or POS convenience. The buyer is responsible for the whole venue and should see VoyceLab as an AI operations employee they can talk to.",
      "Use current public information only to understand owner pains. Prefer wedding venues, event venues, private-event spaces, banquet/event properties and multi-location venue groups. Bars or breweries are secondary only when events are a meaningful part of the business.",
      "Customer-facing language must be blunt, concrete and understandable in five seconds. Never use: voice layer, orchestration, operational intelligence, workflow transformation, connected systems, AI-powered operations, streamline, unlock, leverage, optimize, ecosystem, or platform transformation.",
      "Do not claim direct calendar scheduling. Tour/booking inquiries can be handled through configured business email and connected data, but calendar creation is not a confirmed capability yet.",
      ...
      "Each variant should express one owner-level reason to sign up: know what is happening without opening dashboards, get inventory/operations answers instantly, or stay on top of booking and event work from one voice agent.",
